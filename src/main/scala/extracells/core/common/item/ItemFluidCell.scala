...
import extracells.core.common.inventory.{InventoryECFluidConfig, InventoryECUpgrades}
...
object ItemFluidCell extends ItemCellBase with IFluidStorageCell{
  ...
  override def getPreformatted(stack: ItemStack): util.ArrayList[Fluid] = {
    ...
    val tagList = stack.getTagCompound.getTagList("ec:preformatConfig", Constants.NBT.TAG_COMPOUND)
    for (i <- 0 until tagList.tagCount) {
      try {
        Option(tagList.getCompoundTagAt(i))
          .map(_.getString("fluid").substring(InventoryECFluidConfig.strPrefix.length))
          .flatMap(s => Option(FluidRegistry.getFluid(s)))
          .foreach(f => preformatList.add(f))
      } catch {
        case e: IndexOutOfBoundsException => //NO-OP
      }
    }
    preformatList
  }
  ...
  override def addInformation(stack: ItemStack, player: EntityPlayer, list: JavaList[_], bool: Boolean): Unit = {
    ...
    strList.add(String.format(StatCollector.translateToLocal("extracells.tooltip.storage.fluid.bytes"),
      usedBytes.toString, cellHandler.totalBytes().toString))
