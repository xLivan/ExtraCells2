...
abstract class AbstractFluidWidget(val fluidTermGui: TFluidWidgetGui,
                                    ...
                                    fluidTmp: Fluid) extends Gui{
  var fluid: Option[Fluid] = Option(fluidTmp)

  def setFluid(fluid: Fluid): Unit = {
    this.fluid = Option(fluid)
  }

  def setFluid(fluidID: Int): Unit = {
    this.fluid = Option(FluidRegistry.getFluid(fluidID))
  }

  def getFluid: Option[Fluid] = this.fluid

  protected def isPointInRegion(top: Int, left: Int,
                                height: Int, width: Int,
                                pointX: Int, pointY: Int): Boolean = {
    GuiUtil.gridRangeCheck(top - 1 until top + height + 1,
      left - 1 until left + width + 1,
      pointX - this.fluidTermGui.getGuiLeft,
      pointY - this.fluidTermGui.getGuiTop - 18)
  }

  protected def drawHoverText(strs: Seq[String], x: Int, y: Int,
                              fontRender: FontRenderer): Unit = {
    if (strs.isEmpty)
      return
